...
Babel plugin that transforms UI5 to CommonJS. UI5 can be any app built as SAPUI5, OpenUI5 or SAP Fiori.
[Check out the example project](https://github.com/msrose/babel-plugin-transform-ui5-to-commonjs/tree/master/examples/transform-amd-to-commonjs-example#transform-amd-to-commonjs-example), which uses this plugin to allow [jest](https://facebook.github.io/jest/) to synchronously `sap.ui.require` UI5 modules.
...
UI5 modules are interpreted as described by [Modules and Dependencies](https://ui5.sap.com/#/topic/91f23a736f4d1014b6dd926db0e91070).
...
- _All_ calls to `sap.ui.require` where it is given an array of dependencies as its first argument will be transformed.
- Explicitly requiring `require`, `module`, and `exports` in an UI5 module will not generate a call to require, but instead defer to the global require, module, and exports assumed to be in the CommonJS environment you are transforming to.
...
Make sure that you have only one UI5 module defined per file, otherwise you'll experience strange results once transformed to the CommonJS format.
...
- Returning any value other than `undefined` from a factory function will override anything you assign to `module` or `exports`.
  This behaviour is in accordance with the UI5 specification.
...
This is a fork from https://github.com/msrose/babel-plugin-transform-ui5-to-commonjs.
